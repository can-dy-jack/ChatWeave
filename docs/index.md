...
### 1. 适用范围
本隐私政策适用于 涌言 移动应用（以下简称“本应用”）及其相关功能，用于说明我们如何处理与你相关的信息。
...
- 设备与网络必要信息：为保障连接和功能运行，可能涉及基础网络请求信息。

说明：
- 本应用默认将上述核心配置与会话数据保存在你的设备本地。  
- 我们不会主动向自建服务器上传你的聊天内容用于训练。  
- 当你连接第三方 AI 服务时，你的请求数据会发送至你选择的第三方服务商。
...
- 你可以通过应用设置中的“清除缓存/数据”功能删除所有本地存储的信息。
...
- Necessary device/network data: basic request-related data required for connectivity and feature operation.

Notes:
- Core configuration and conversation data are stored locally on your device by default.  
- We do not proactively upload your chat content to our own servers for model training.  
- When you connect to third-party AI providers, your request data is transmitted to those providers.
...
- You can delete all locally stored information via the "Clear cache/data" feature in the App settings.
